...
export default function Collections() {
 ...
 {state.loading && !state.rows.length ? <Loading/> : <div className="record-list">{rows.map(row=><article className="record-card followup-card status-unreachable" key={row.id}><div className="record-main"><Link to={`/customers/${row.id}`}><h3>{row.name}</h3></Link><p>{row.area || row.branchId || ""} ? Assigned Staff: {row.assignedStaffName || (row.assignedStaffId === profile?.uid ? profile?.name : "Awaiting assignment")}</p><div className="collection-numbers compact"><div><span>Due after buffer</span><strong className="danger-text followup-highlight">{money(row.due.amount)}</strong></div></div><p>Unpaid records: {row.due.invoices.length} ? Due dates: {[...new Set(row.due.invoices.map(invoice=>invoice.dueDate))].join(", ")} ? Buffer days: {[...new Set(row.due.invoices.map(invoice=>String(invoice.bufferDays)))].join(", ")}</p><small>Invoice/payment data synced {when(row.scheduleUpdatedAt)}</small></div><button className="secondary" onClick={()=>setRecord(row)}>Record promise</button></article>)}</div>}
