...
export default function ManagerHome() {
 ...
 const followups=useRows("followUps");
 ...
 <ErrorBox message={tasks.error || followups.error || team.error || customers.error || staffCustomers.error}/>
 ...
 {tab === "followups" && <Panel title="Branch follow-ups"><div className="record-list">{followups.rows.map(row=><article className={`record-card followup-card status-${String(row.status||"").toLowerCase()}`} key={row.id}><h3>{row.title}</h3><p className="followup-highlight">Last order: {(() => { const c=staffCustomers.rows.find(x=>x.id===row.customerId); return c?.lastOrderDate ? `${Math.max(0,Math.floor((Date.parse(today()+"T00:00:00Z")-Date.parse(String(c.lastOrderDate)+"T00:00:00Z"))/86400000))} days` : "unknown"; })()}</p><Badge value={row.status}/>{row.notes && !String(row.notes).startsWith("Automatically created") && <p>{row.notes}</p>}</article>)}</div><PageEnd state={followups}/></Panel>}
