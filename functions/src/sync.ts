...
export async function syncStep(
  ...
  if (process.env.FIRESTORE_EMULATOR_HOST && !testReader)
  ...
  try {
    ...
    if (state.phase === "READ" || state.mode === "INVOICE_TARGET")
    ...
    if (state.mode === "INVOICE_TARGET") {
      ...
      if (dirty.size < 25) state.phase = "FINALIZE";
    } else {
      await rebuildTargets(state.month);
      state = {
        ...state,
...
export async function refreshCustomer(id: string) {
  const [c, credit, assignment] = await Promise.all([
