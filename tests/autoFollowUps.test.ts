...
describe("Automatic 15-day follow-up policy", () => {
  it("includes exactly fifteen days and older, but not fourteen days", () => {
    expect(needsAutomaticFollowUp("2026-08-27", "2026-09-06")).toBe(true);
    expect(needsAutomaticFollowUp("2026-08-26", "2026-09-06")).toBe(true);
    expect(needsAutomaticFollowUp("2026-08-28", "2026-09-06")).toBe(false);
  });
  ...
  it("handles calendar boundaries and invalid dates", () => {
    expect(followUpDueDate("2024-02-20", "2024-03-01")).toBe("2024-03-01");
  ...
  it("uses stable IDs without colliding with manual follow-ups", async () => {
    ...
    expect(id.startsWith("auto10_")).toBe(true);
  });
});
